...
class PDFGenerator:
    ...
    def _map_post_to_slides(
        self, post_data: Dict[str, Any], author: str = "Aditya Kulkarni"
    ) -> List[Dict[str, Any]]:
        slides = []
        ...
        for i, section in enumerate(sections):
            ...
            if not content:
                ...
                elif section.get("tradeoffs") and section["tradeoffs"].get(
                    "real_world_context"
                ):
                    content = section["tradeoffs"]["real_world_context"]

            is_quote = (content.startswith('"') and content.endswith('"')) or (
